        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-700/60 rounded-lg py-2 pl-9 pr-10 text-sm focus:outline-none focus:border-amber-500/50 text-slate-200 transition-all"
              disabled={loading}
            />
            {localSearch && (
              <button
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-slate-950/80 rounded-lg p-1 border border-slate-800">
            <button
              ...
              className={`p-1.5 rounded transition-all ${viewMode === 'list' ? 'bg-slate-800 text-amber-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
              disabled={loading}
            >
              <LayoutList size={16} />
            </button>
            <button
              ...
              className={`p-1.5 rounded transition-all ${viewMode === 'card' ? 'bg-slate-800 text-amber-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
              disabled={loading}
            >
              <LayoutGrid size={16} />
            </button>
          ...
          <button
            onClick={() => openModal()}
            className="flex items-center gap-2 bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg font-medium text-sm"
            disabled={loading}
          >
      ...
      <div className="flex-1 overflow-auto custom-scrollbar">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 text-slate-500">
            <Loader2 size={48} className="animate-spin mb-4 text-amber-400" />
          ...
          viewMode === 'card' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-6">
              {categories.map(category => (
                <div key={category.id} className="relative bg-slate-900 border border-slate-800/80 rounded-xl hover:border-amber-500/40 hover:bg-slate-800/40 transition-all group flex flex-col min-h-[160px] overflow-hidden shadow-sm hover:shadow-xl hover:shadow-amber-950/10">
                  <div className="h-1 w-full bg-amber-500 opacity-30 group-hover:opacity-100 transition-opacity"></div>
                  <div className="p-4 flex flex-col flex-1">
                    <div className="flex justify-between items-start mb-3">
                      <div className="p-2 rounded-lg bg-slate-950 border border-slate-800 text-amber-400 group-hover:text-amber-300 transition-colors">
                        <FolderOpen size={18} />
                      </div>
                    </div>
                    <div className="mb-2">
                      <h4 className="text-sm font-bold text-white group-hover:text-amber-400 transition-colors">{category.name}</h4>
                    </div>
                    ...
                    <div className="mt-auto pt-3 border-t border-slate-800/40 flex justify-between items-center">
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="p-1.5 hover:bg-slate-700/50 rounded-lg text-slate-500 hover:text-amber-400 transition-all"
                          title="Edit"
                        >
                          <Settings size={14} />
                        </button>
                ...
                <tbody className="divide-y divide-slate-800 bg-slate-900/40">
                  {categories.map(category => (
                    <tr key={category.id} className="hover:bg-slate-800/40 transition-colors group">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="p-1.5 rounded-lg bg-slate-950 border border-slate-800 text-amber-400">
                            <FolderOpen size={14} />
                          </div>
                          <span className="text-sm font-bold text-white group-hover:text-amber-400 transition-colors">{category.name}</span>
                        </div>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button
                            onClick={() => openModal(category)}
                            className="p-2 hover:bg-slate-700 rounded-lg text-slate-500 hover:text-amber-400"
                            title="Edit"
                          >
                            <Settings size={16} />
                          </button>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-slate-500 bg-slate-900/20 border border-dashed border-slate-800 rounded-2xl">
            ...
            <button
              onClick={() => openModal()}
              className="flex items-center gap-2 bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded-lg transition-all text-xs font-bold"
            >
...
const CategoryFormModal: React.FC<CategoryFormModalProps> = ({ category, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border-t-4 border-t-amber-600">
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input
                ...
                className={`w-full bg-slate-950 border rounded-lg p-2.5 text-sm text-white focus:outline-none transition-all ${
                  errors.name ? 'border-red-500/50' : 'border-slate-700 focus:border-amber-500/50'
                }`}
            ...
            <div>
              ...
              <textarea
                ...
                className={`w-full bg-slate-950 border rounded-lg p-2.5 text-sm text-white focus:outline-none resize-none transition-all ${
                  errors.description ? 'border-red-500/50' : 'border-slate-700 focus:border-amber-500/50'
                }`}
          ...
          <div className="p-5 bg-slate-950/30 border-t border-slate-800 flex justify-end gap-3">
            ...
            <button
              type="submit"
              className="px-6 py-2.5 bg-amber-600 hover:bg-amber-500 text-white text-xs font-black tracking-widest rounded-lg shadow-lg shadow-amber-900/20 transition-all flex items-center gap-2 disabled:opacity-50"
              disabled={isSubmitting}
            >
              {isSubmitting ? (
                <>
